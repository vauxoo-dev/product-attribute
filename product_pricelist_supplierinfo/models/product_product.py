...
class ProductProduct(models.Model):
    ...
    def _select_seller(
        ...
        for seller in sellers:
            if (
                (not seller.date_start or fields.Date.context_today(seller, seller.date_start) <= date)
                and (not seller.date_end or fields.Date.context_today(seller, seller.date_end) >= date)
                and (not partner_id
